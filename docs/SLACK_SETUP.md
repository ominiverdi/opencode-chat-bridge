...
Restrict the bot to specific Slack users with `SLACK_ALLOWED_USERS` or `slack.allowedUsers` in `chat-bridge.json`.
...
To find a Slack user ID: click profile picture, then copy member ID from the profile menu.
